...
| **Version** | 1.0.0+1 |
| **Repo note** | Package/folder may still be named `seatcare_app`; **all user-facing branding is Waby** |
...
5. **Brand = Waby** — navy/teal calm UI; no “SeatCare” in user-facing copy.
...
| AUTH-6 | Greeting name prefers nickname → full name → `"there"` | Should |
...
| PIC-5 | Support remove photo (null path) and live-update Home header avatar | Should |
...
| Home | Live dashboard, greeting + avatar, add device, child cards |
| Family (`ContactsScreen`) | Children (edit/delete), members (RO), emergency contacts + link codes, join code |
| Settings | Timer, tests, family management, privacy/help, auth actions |
| Profile | Edit caregiver details + avatar |
...
| Colour source | `lib/core/theme.dart` — Primary navy `#0F2D54`, accent `#3B74BC`, wave `#4696B7`→`#4C9DBC`, SAFE `#56B337`, WARNING `#C2291D` |
| Patterns | Wave headers, status pills, child cards tinted by severity, navy primary buttons |
...
| Vibration / audible toggles | Local UI only |
...
- Wire distance/audio preference toggles to firmware or alert service intentionally
