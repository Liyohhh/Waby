...
| ≥ −78 dBm | Near |
| ≤ −90 dBm | Far |
| Beacon lost for 20s (after it was seen) | Far |

RSSI is an 11-sample rolling median. Near commits after 2 consistent readings; Far after 15. Until the beacon is seen once this session, `live.distance_near` is left unchanged (avoids false left-behind if firmware is not yet flashed). The app PATCHes `distance_near` when the value changes.
